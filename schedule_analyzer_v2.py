...
async def _parse_one_async(executor, name: str, path: str):
    ...
    occ, issues = await loop.run_in_executor(executor, parse_xlsx, path)
...
def collect_xlsx_from_files(uploaded_files) -> Tuple[str, List[Tuple[str, str]]]:
    ...
    for uf in uploaded_files:
        ...
        with open(dest, "wb") as f:
            f.write(uf.getvalue())
        if uf.name.endswith(".xlsx") and not uf.name.startswith("~$"):
    ...
    with st.sidebar:
        ...
        if upload_mode == "ZIP 压缩包":
            ...
            uploaded_files = st.file_uploader(
                "批量选择 XLSX 文件（可多选，也可整个文件夹拖入）",
                type=["xlsx"],
                accept_multiple_files=True,
                help="可框选整个文件夹中的 xlsx 文件后一次性上传",
            )
...
- 📑 趋势图 PNG
    ...
    try:
        ...
        if not all_paths:
            st.error("未找到任何有效的 .xlsx 课表文件，请检查上传内容。")
            return
